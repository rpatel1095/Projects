...
def openFile(fileName):
    try:
        hostnameFullRS = [line.strip() for line in open(fileName)]
        return hostnameFullRS
...
def server():
    firstTime = 1
    csockid,ss = serverConnect(51050)

    # ******* UNUSURE HOW THEY WILL GRADE ********
    # Gets a list of keys
    inputEduFile = 'PROJ3-KEY2.txt'
    # inputEduFile = sys.argv[1]
    listOfKeys = openFile(inputEduFile)

    #Open List of Hostname
    hostNameListFull = openFile('PROJ3-TLDS2.txt')
    ...
    while True:
        ...
        if(matchResult == "Match"):
            ...
            if(firstTime == 1):
                csockid2, ss2 = serverConnect(52050)
